...
public class PaymentControler {
    ...
    public CommonResult create(@RequestBody Payment dept){
        ...
        logger.info("***************插入成功*******"+i);
        if(i>0){
            return new CommonResult(200,"pro_provide01 插入数据库成功,端口為："+port,null);
        }else{
            return new CommonResult(444,"pro_provide01 插入数据库失败,端口為："+port,null);
        }
    }
    ...
    public CommonResult queryById(@PathVariable("id") Long id){
        ...
        Logger logger = Logger.getLogger("PaymentControler");
        logger.info("***************进入pro_provide01*********");
        logger.info("***************查询成功*********"+payment.toString());
        CommonResult commonResult = null;
        if(payment!=null){
            commonResult = new CommonResult(200, "pro_provide01 查询成功,端口為："+port, payment);

        }else{
            commonResult = new CommonResult(444,"pro_provide01 查询失败,端口為："+port,null);
        }
    ...
    public  Object discovery(){
        ...
        List <ServiceInstance> instances = discoveryClient.getInstances("qlpro-comsumer01");
